...
const iconSizes = { default: 'w-5 h-5', large: 'w-7 h-7' } as const
...
const socialLinks = [
  {
    ...
    icon: (size: keyof typeof iconSizes) => (
      <svg className={iconSizes[size]} fill="currentColor" viewBox="0 0 24 24" aria-hidden>
  ...
  },
  {
    ...
    icon: (size: keyof typeof iconSizes) => (
      <svg className={iconSizes[size]} fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden>
...
type SocialLinksProps = { size?: 'default' | 'large' }

export function SocialLinks({ size = 'default' }: SocialLinksProps) {
  const gapClass = size === 'large' ? 'gap-6' : 'gap-4'
  return (
    <div className={`flex items-center ${gapClass}`}>
      {socialLinks.map((link) => (
        <motion.a
          ...
          className="text-light/60 hover:text-amber-light transition-colors"
